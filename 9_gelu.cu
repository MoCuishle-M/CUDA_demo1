...
template<typename T>
T fast_gelu(T x) {
  const T c = static_cast<T>(0.044714998453855515);
  const T sqrt_2_over_pi = static_cast<T>(0.7978845608028654);
  return static_cast<T>(0.5) * x * (static_cast<T>(1) + std::tanh(sqrt_2_over_pi * (x + c * x * x * x)));
}
...
struct GeluFunctor<half> {
  ...
  static __device__ void Apply2(half* y, const half* x) {
...
int main() {
	...
	// lambda function, check whether the pointer is aligned
    auto is_aligned = [](const void* p, int alignment) {
        return reinterpret_cast<uintptr_t>(p) % alignment == 0;
    };
    // 获取AlignedVector所需的对齐 boundary（对齐边界）
